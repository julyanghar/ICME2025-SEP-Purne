...
if __name__ == '__main__':
    ...
    conf = torch.load(
        '/home/zhaoyu/projects/code/Look2hear/Experiments/checkpoint/AFRCNN_2/best_model.pth', map_location=device
    ...
    # 创建示例输入数据
    input_data = torch.randn((1,1,16000), device=device)
    apply_custom_pruning(model, input_data, s=0.05)
    measure_inference_time(model)
    ...
    conf["state_dict"] = model.state_dict()
    # 保存剪枝后的模型
    torch.save(conf, "/home/zhaoyu/projects/code/Look2hear/Experiments/checkpoint/AFRCNN_2/pruned_afrcnn_model_005.pth")
